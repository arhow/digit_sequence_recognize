...
import numpy as np
from digit_sequence_recognize.digit_sequence_generator.mnist import MNIST
...
class MNIST_SEQUENCE(object):
    ...
    def __calculate_uniform_spacing(self, size_sequence, minimum_spacing, maximum_spacing,
        ...
        if size_sequence <= 1:
            return 0
        allowed_spacing = int((total_width - size_sequence * image_width) / ((size_sequence - 1) * 1.0))
        if type(allowed_spacing)!=int or allowed_spacing < minimum_spacing \
    ...
    def generate_image_sequence(self, sequence, minimum_spacing, maximum_spacing, total_width, image_height):
        sequence_length = len(sequence)
        allowed_spacing = self.__calculate_uniform_spacing(sequence_length, minimum_spacing, maximum_spacing, total_width, image_height)
        spacing = np.ones(image_height * allowed_spacing, dtype='float32').reshape(image_height, allowed_spacing)
